...
export default function Navigation() {
  ...
  const navLinkProps = (sec: string) => {
    if (isHome) {
      return { as: 'a' as const, href: `#${sec}` };
    }
    return { as: Link as any, to: '/', state: { scrollTo: sec } };
  };

  const brandLinkProps = isHome
    ? { as: 'a' as const, href: '#intro' }
    : { as: Link as any, to: '/' };
  ...
  return (
    <>
          ...
          >
            <motion.div className="h-full flex flex-col justify-center items-start px-8 pb-8">
              <div className="space-y-6 w-full">
                {sections.map(sec => {
                  const { as: Comp, ...linkProps } = navLinkProps(sec);
                  return (
                    <motion.div
                      key={sec}
      ...
      <motion.div
        ...
        className={`lg:hidden fixed top-0 left-0 right-0 z-[101] flex items-center justify-between px-5 py-4 transition-all duration-500 ${isScrolled || isMenuOpen || !isHome ? 'border-b border-white/10 bg-[#040810]/70 backdrop-blur-md' : 'bg-transparent'}`}
      >
        {isHome ? (
          <a
            href="#intro"
